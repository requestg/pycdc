...
int main(int argc, char* argv[])
{
    ...
    if (!mod.isValid()) {
        fprintf(stderr, "غير قادر على التحليل %s\n", infile);
        return 1;
    }
    const char* dispname = strrchr(infile, PATHSEP);
    dispname = (dispname == NULL) ? infile : dispname + 1;
    *pyc_output << "# Decode By Pycdc Sajad\n";
    formatted_print(*pyc_output, "# File : %s (version python %d.%d%s)\n\n", dispname,
                    mod.majorVer(), mod.minorVer(),
                    (mod.majorVer() < 3 && mod.isUnicode()) ? " Unicode" : "");
    try {
        decompyle(mod.code(), &mod, *pyc_output);
    } catch (std::exception& ex) {
        fprintf(stderr, "خطا في الفك %s: %s\n", infile, ex.what());
        return 1;
    }
